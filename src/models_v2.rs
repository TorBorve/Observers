...
impl<T, const NX: usize, const NW: usize, const NY: usize, const NV: usize> Differentiable<NX, NW, NY, NV> for T
where
...
impl<const NX: usize, const NY: usize, const NU: usize> ObserverModel<NX, NU, NX, NY, NU, NY> for GaussianLinearSystem<NX, NY, NU> {
    fn state_model(
        &self,
        x: &na::SVector<f64, NX>,
        u: &na::SVector<f64, NU>,
    ...
    fn gen_state_noise(&self, num_noise_samples: usize) -> Vec<na::SVector<f64, NX>> {
        let mut noise_sampler = multivariate_normal_sampler(&na::SVector::<f64, NX>::zeros(), &self.w_cov);
    ...
    fn gen_meas_noise(&self, num_noise_samples: usize) -> Vec<na::SVector<f64, NY>> {
        let mut noise_sampler = multivariate_normal_sampler(&na::SVector::<f64, NY>::zeros(), &self.v_cov);
        let noise_samples: Vec<_> = (0..num_noise_samples).map(|_| noise_sampler()).collect();
        assert_eq!(noise_samples.len(), num_noise_samples);
        noise_samples
    }
